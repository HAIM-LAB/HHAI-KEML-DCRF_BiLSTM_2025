...
def train_model(model, X_train, y_train, X_test, y_test, epochs=200, dataset_name="None", lr=0.0001):
    ...
    #featY = to_categorical(featY, num_classes=NUM_CLASSES)
    ...
    #history = model.fit(X_train, y_train, epochs=epochs, batch_size=256, validation_data=(X_test, y_test),callbacks=[best_model_callback],verbose=0)
    ...
    #history = model.fit(X_train, y_train, epochs=epochs, batch_size=256, validation_data=(X_test, y_test), callbacks=[best_model_callback, CustomEpochCallback])
...
def train_DeepCRF(dataset, dataset_name):
    ...
    print("X_train.shape='{}', dim='{}'".format(X_train.shape,X_train.ndim))
    # print("X_test.shape='{}', dim='{}'".format(X_test.shape,X_test.ndim))
    scaler = StandardScaler()
    X_train = scaler.fit_transform(X_train)
    X_test = scaler.transform(X_test)
    #X_val = scaler.transform(X_val)
    X_feat = scaler.transform(X)
    print("STd:X_train.shape='{}', dim='{}'".format(X_train.shape,X_train.ndim))
    # print("STD:X_test.shape='{}', dim='{}'".format(X_test.shape,X_test.ndim))
    ...
    print("After PCA :X_train.shape='{}', dim='{}'".format(X_train.shape,X_train.ndim))
    print("Final feature count is '{}'".format(X_train.shape[1]))
...
def save_history(history, dataset_name):
    ...
    #history_df.to_csv(os.path.join(output_dir, "{dataset_name}_metrics_training_history.csv"), index=False)
